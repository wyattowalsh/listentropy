...
function compareStatusLabel(status: LabModuleStatus): string {
  switch (status) {
...
function compareStatusTone(status: LabModuleStatus): string {
  switch (status) {
...
function renderModeAwareCompareSection(
  ...
  if (isSimpleMode) {
    return (
      <details className="mt-3 rounded-theme border border-border bg-surface-hover p-3">
        <summary className="cursor-pointer text-xs text-text-muted">{title}</summary>
        <div className="mt-3">{content}</div>
      </details>
    )
  }
  return <div className="mt-3 rounded-theme border border-border bg-surface-hover p-3">{content}</div>
}
...
export function CompareWorkspacePanel({
  ...
  const deepComparePanels = payload ? (
    <>
    ...
    <Card className="min-w-0">
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-3">
        <div className="rounded-theme border border-border bg-surface-hover p-2">
          <p className="text-[11px] uppercase tracking-[0.12em] text-text-muted">Compare status</p>
          <span className={`mt-1 inline-flex rounded-theme border px-2 py-1 text-[11px] uppercase tracking-[0.12em] ${compareStatusTone(compareStatus)}`}>
            {compareStatusLabel(compareStatus)}
          </span>
        </div>
        <div className="rounded-theme border border-border bg-surface-hover p-2">
          <p className="text-[11px] uppercase tracking-[0.12em] text-text-muted">Import mode</p>
          <p className="mt-1 text-sm capitalize text-text">{importMode}</p>
        </div>
        <div className="rounded-theme border border-border bg-surface-hover p-2">
          <p className="text-[11px] uppercase tracking-[0.12em] text-text-muted">Saved snapshots</p>
          <p className="mt-1 text-sm text-text">{savedSnapshots.length}</p>
        </div>
      </div>
      ...
      <details className="mt-3 rounded-theme border border-border bg-surface-hover p-3" open={!isSimpleMode}>
        <summary className="cursor-pointer text-xs text-text-muted">Saved Compare Snapshots</summary>
          ...
          {deepComparePanels ? (
            isSimpleMode ? (
              <details className="rounded-theme border border-border bg-surface-hover p-3">
                ...
                <div className="mt-3 space-y-3">{deepComparePanels}</div>
              </details>
            ) : (
              deepComparePanels
            )
