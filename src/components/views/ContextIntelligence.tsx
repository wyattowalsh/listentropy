...
export function ContextIntelligence({ data }: ContextIntelligenceProps): JSX.Element {
  ...
  const topReasons = contextAnalytics.reasons.transitions.slice(0, 10)
  const topDeviceTransitions = contextAnalytics.deviceJourney.transitions.slice(0, 10)
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <Card>
          ...
          <ol className="mt-3 space-y-2">
            {topReasons.map((item) => (
              <li key={`${item.from}-${item.to}`} className="flex items-center justify-between text-sm">
